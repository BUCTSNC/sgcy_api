import { React, useEffect, useNavigate, useParams, useSearchParams, useState } from "../../deps/react.ts";
...
function SearchBar() {
    ...
    return <div>
        ...
        }} value={keywords.join(" ")} placeholder="请输入关键词进行搜索" />
        <button>搜索</button>
    </div>
}
...
function Navigation() {
    ...
    return <div className={categoryNavibar}>
        {
            (["校园生活", "服务指南", "学习资源", "校园风光"]).map(
                (categoryName, index) => <div key={index} onClick={() => navi(`/cate/${categoryName}`)} className={categoryName===activedCate ? activeCateTab : inactiveCateTab}>{categoryName}</div>
            )
        }
    </div>
}
